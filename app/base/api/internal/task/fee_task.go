...
func (t *FeeTask) Start() {
	go t.readPriorityFee()
	go t.updatePerUnitFee()
	go t.updatePerTxFee()
	go t.startEstimateWeightAvgFee()
}
...
func (t *FeeTask) readPriorityFee() {
	...
	for {
		func() {
			// 分布式锁，防止重复查询导致重复处理交易
			scanMutex := t.redSync.NewMutex("SOL-STATIST-FEE-ON-BLOCKCHAIN")
...
func (t *FeeTask) updatePerUnitFee() {
	...
	for {
		func() {
			...
			table := t.db.Table(model.TableNameFeeStatistics)
			// 分布式锁，防止重复查询导致重复处理交易
			mutex := t.redSync.NewMutex("SOL-READ-PRIORITY-FEE-PER-COMPUTE-UNIT-IN-DATABASE")
			...
			// 将整个 JSON 数组存入 Redis
			t.redis.Set(ctx, "SOL-PRIORITY-FEE-PER-COMPUTE-UNIT-ON-BLOCKCHAIN", jsonData, 0).Err()
		}()
...
func (t *FeeTask) updatePerTxFee() {
	...
	for {
		func() {
			...
			table := t.db.Table(model.TableNameFeeStatistics)
			// 分布式锁，防止重复查询导致重复处理交易
			mutex := t.redSync.NewMutex("SOL-READ-PRIORITY-FEE-PER-TRANSACTION-UNIT-IN-DATABASE")
			if err := mutex.Lock(); err != nil {
				return
			}
			defer mutex.Unlock()
			...
			// 将整个 JSON 数组存入 Redis
			t.redis.Set(ctx, "SOL-PRIORITY-FEE-PER-TRANSACTION-ON-BLOCKCHAIN", jsonData, 0).Err()
		}()
...
func (t *FeeTask) startEstimateWeightAvgFee() {
	url := "https://nameless-old-spring.solana-mainnet.quiknode.pro/a30ccaa45b9570bcfce344407013972849ae0484/"
	maxRecordNum := 20

	for {
		// 将锁操作包裹在匿名函数中，确保defer生效
		func() {
			// 获取分布式锁（避免重复计算手续费）
			scanMutex := t.redSync.NewMutex("SOL-ESTIMATE-WEIGHT-AVG-Fee")
			if err := scanMutex.Lock(); err != nil {
				var errTaken *redsync.ErrTaken
				if errors.As(err, &errTaken) {
					return // 过滤"锁已被占用"的预期错误[2](@ref)
				}
				log.Errorf("获取统计锁失败: %v", err)
				return
			}
			defer scanMutex.Unlock() // 确保锁释放
			...
			if jsonData, err := json.Marshal(feeDetail); err == nil {
				ctx := context.Background()
				_ = t.redis.Set(ctx, "SOL-WEIGHT-AVG-FEE-PER-COMPUTE-UNIT", jsonData, 0)
				_ = t.redis.Set(ctx, "SOL-WEIGHT-AVG-FEE-PER-TRANSACTION", jsonData, 0)
			} else {
